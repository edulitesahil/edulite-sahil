...
public class MatrixInverse {
    ...
    // A method to calculate the inverse of a matrix
    public static double[][] inverse(double[][] matrix) {
        int n = matrix.length; // the dimension of the matrix
        double det = determinant(matrix); // calculate the determinant
        if (det == 0) { // check if the matrix is singular
            System.out.println("Inverse does not exist"); // print an error message
            return null; // return null
        }
        double[][] inv = new double[n][n]; // the inverse matrix
        double[][] adj = adjoint(matrix); // calculate the adjoint
        for (int i = 0; i < n; i++) { // loop through the rows
            for (int j = 0; j < n; j++) { // loop through the columns
                // divide the adjoint element by the determinant
                inv[i][j] = adj[i][j] / det;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        double[][] inv = inverse(matrix); // calculate the inverse
        if (inv != null) { // check if the inverse exists
            System.out.println("The inverse of the matrix is: "); // print the result
            for (int i = 0; i < n; i++) { // loop through the rows
                for (int j = 0; j < n; j++) { // loop through the columns
                    System.out.print(inv[i][j] + " "); // print the element
                }
